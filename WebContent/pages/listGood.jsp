...
<title>查找货物</title>
    ...
    function deletegood(id){
    	  ...
    	  if(b){
    		  window.location.href="${pageContext.request.contextPath}/Good/delete/"+id;
    	  }
      }
    </script>
                    ...
                    <div class="col-lg-12">
                        <div class="panel panel-default">
                            <div class="panel-heading">
                                                         请选择所要查找的货物种类：
                       <form class="form-inline" action="${pageContext.request.contextPath }/Good/findforCategory" method="post">                       
                               <div class="form-group">                                              
                                                <select class="form-control"  name="category">   
                                                     <option value="">--请选择--</option>                                                  
                                                   <c:forEach var="category"  items="${categories}">
    					                             <option value="${category.id }"<c:if test="${category.id == category_id}"> selected</c:if>>${category.name }</option>
    				                               </c:forEach>
                                                </select>                                               
                               </div>
                               <div class="form-group">
                               ...
                               <input type="button" name="test" value="查找全部" onClick="location.href='${pageContext.request.contextPath }/Good/listgood'"/>
                  </form>                                                          
                           </div>
                            <!-- /.panel-heading -->
                                ...
                                <table width="100%" class="table table-striped table-bordered table-hover" id="dataTables-example1" style="table-layout:fixed;">
                                    ...
                                    <c:forEach var="good" items="${goods}">
   			                           <tr>
   				                        ...
   				                        <td>${good.quantity}</td>
   				                        <td>${good.location}</td>  				                     				
   				                        <td><a href="${pageContext.request.contextPath }/images/${good.image }" target="_blank">查看图片</a></td>
   				                        <td>${good.description}</td>
   				                        <td>${good.categoryname}</td>
	   			                        <td>
	   				                     <a href="${pageContext.request.contextPath}/Good/editgood/${good.id}">修改</a>
	   				                     <a href="javascript:void(0)" onclick="deletegood('${good.id}')">删除</a>
	   			                         </td>
   			                            </tr>
   		                            </c:forEach>   		       			                               
                                    </tbody>
    ...
    <script>
        $(document).ready(function() {
            $('#dataTables-example1').DataTable({
                responsive: true,
                "aoColumnDefs": [ { "bSortable": false, "aTargets": [ 0,4,5,6 ] }] //0456列不排序
            });
        });
     /*   $(function() {
